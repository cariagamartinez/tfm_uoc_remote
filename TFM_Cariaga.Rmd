...
#Algunas características de la base de datos original
summary(bbddAECM)

char_cols <- bbddAECM %>%
  dplyr::select(where(is.character))

#Creamos tabla de frecuencias en formato largo
freq_table <- char_cols %>%
  pivot_longer(everything(), names_to = "column", values_to = "value") %>%
  group_by(column) %>%  # Agrupar por columna
  count(value, name = "Frecuencia") %>%
  mutate(Porcentaje = round(Frecuencia / sum(Frecuencia) * 100, 2)) %>%  # Porcentaje por columna
  ungroup()
...
mann_results <- sapply(colnames(numeric_vars_log)[-which(colnames(numeric_vars_log) == "Target")], function(var) {
